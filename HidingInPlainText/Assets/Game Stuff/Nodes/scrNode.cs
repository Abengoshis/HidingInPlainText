...
public class scrNode : MonoBehaviour
{
	...
	IEnumerator ConstructTitan()
	{
		while (!ready)
			yield return new WaitForEndOfFrame();

		GameObject titan = (GameObject)Instantiate (scrEnemyMaster.Instance.EggTitanPrefab, transform.position, transform.rotation);

		float titanStartScale = 1.0f / 50.0f;
		for (float t = 0.0f, tEnd = 60.0f; t < tEnd; t += Time.deltaTime)
		{
			float tFactor = t / tEnd;

			// Scale up the titan over time.
			titan.transform.localScale = Vector3.one * Mathf.SmoothStep(titanStartScale, 1.0f, tFactor);

			// Scale up and fade out the orb.
			ChildOrb.transform.localScale = Vector3.one * Mathf.SmoothStep(1.0f, 30.0f, tFactor);
			ChildOrb.renderer.material.color = Color.Lerp (scrNodeMaster.INFECTED_ORB_COLOUR, Color.clear, tFactor);

			// Move the cubes out to contain the titan in a sphere.
			// Expand out cubes.
			for (int i = 0; i < Cubes.Length; ++i)
			{
				Vector3 position = transform.TransformPoint(cubePositionEnumerator.Current[i]);
				Cubes[i].Value.transform.position = Vector3.Lerp (position, transform.position + (position - transform.position).normalized * 30.0f, Mathf.SmoothStep(0.0f, 1.0f, tFactor));
			}
		...
		titan.GetComponent<scrTitanEnemy>().Activate();
